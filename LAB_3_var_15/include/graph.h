...
public:
    ...
    void walk(const Vertex& start_vertex, std::function<void(const Vertex&)> action) const {
        ...
        processed[std::distance(_vertices.begin(), std::find(_vertices.begin(), _vertices.end(), start_vertex))] = true;
        ...
        while (!queue.empty()) {
            ...
            for (const auto& edge : _edges.at(current)) {
                size_t index = std::distance(_vertices.begin(), std::find(_vertices.begin(), _vertices.end(), edge.to));
                if (!processed[index]) {
                    queue.push(edge.to);
